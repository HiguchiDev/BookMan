...
Mimic::Mimic(std::string fileName, std::shared_ptr<Position> position, std::shared_ptr<TurnController> turnController) : Enemy(fileName, position, turnController)
{
	this->_characterName = "Mimic";
	this->_active = false;
...
void Mimic::actionOfCollision(std::shared_ptr<HitBox> hitBox)
{
	if (this->_active){
