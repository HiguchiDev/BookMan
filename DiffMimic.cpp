...
DiffMimic::DiffMimic(std::string fileName, std::shared_ptr<Position> position, std::shared_ptr<TurnController> turnController) : Enemy(fileName, position, turnController)
{
	this->_characterName = "DiffMimic";
	this->_active = false;
...
bool DiffMimic::beExamined()
{
	if (this->_active == false){
		MessgeBox::getInstance()->add("たからばこはミミックだった！！！");
		this->_active = true;
		return true;
	}
	else{
...
void DiffMimic::actionOfCollision(std::shared_ptr<HitBox> hitBox)
{
	if (this->_active){
